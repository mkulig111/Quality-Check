...
import { collection, query, where, getDocs, Timestamp, orderBy, limit, addDoc, serverTimestamp, writeBatch } from "firebase/firestore";
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import { Info } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { Label } from "@/components/ui/label";
import { spcFromStorageFlow } from "@/ai/flows/spc-from-storage-flow";

interface SpcOption {
    id: string;
    label: string;
    checksheetId: string; // In the new logic, this will be the Item Name for matching
    checksheetDocId: string; // The actual firestore doc id
    fieldName: string;
    lsl?: number;
    usl?: number;
}
...
export function SpcTab() {
  ...
  const handleDepartmentSelect = async (department: string) => {
    ...
    try {
      ...
      querySnapshot.forEach((doc) => {
          sheets.push({ id: doc.id, ...(doc.data() as Omit<Checksheet, 'id'>) });
      });
      setDepartmentChecksheets(sheets);
    } catch (error) {
      toast({ title: "Error", description: "Could not fetch check sheets for the department.", variant: "destructive" });
  ...
  const handleGenerateClick = async () => {
    if (!selectedSpcOption || !selectedDepartment || !date?.from) {
        toast({ title: "Missing selection", description: "Please select department, characteristic, and date range.", variant: "destructive"});
        return;
    }
    ...
    try {
        ...
        const result = await spcFromStorageFlow({
            department: selectedDepartment,
            checksheetId: selectedOption.checksheetId, // This is now Item Name
            ...
            numberOfPoints: pointsToFetch
        });
        
        setAvailableData(result.foundPoints);

        if (result.values.length >= pointsToFetch) {
            setSpcData({
  ...
  const handleSeedData = async () => {
    ...
    const checksheetQuery = query(collection(firestore, "checksheets"), where("itemName", "==", "Housing CTR (MC9 - 2500T)"), limit(1));
    const querySnapshot = await getDocs(checksheetQuery);

    if (querySnapshot.empty) {
        toast({
            title: "Seeding Failed",
            description: "Checksheet 'Housing CTR (MC9 - 2500T)' not found. Please create it first.",
            variant: "destructive",
        });
        setIsSeeding(false);
        return;
    ...
    if (!specialField) {
         toast({
            title: "Seeding Failed",
            description: "No special characteristic found for 'Housing CTR (MC9 - 2500T)'.",
            variant: "destructive",
        });
        setIsSeeding(false);
        return;
    ...
    const stdDev = (usl - lsl) / 6; // Approx. for a capable process
    ...
    for (let i = 0; i < 30; i++) {
        ...
        const measurement = {
            [specialField.fieldName]: parseFloat(randomValue.toFixed(3))
        };
        ...
        const docPayload = {
            ...
            department: checksheet.department, // Add department to seeded data
            machine: (checksheet as any).machine || 'N/A', // Add machine
  ...
  const handleClearSeededData = async () => {
    ...
    try {
        ...
        if (querySnapshot.empty) {
            toast({
                title: "No Seeded Data Found",
                description: "There is no seeded data to clear.",
            });
            setIsClearing(false);
            return;
        }
        ...
        toast({
            ...
            description: "An unexpected error occurred while clearing seeded data.",
            variant: "destructive",
        });
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>SPC Analysis from Cloud Storage</CardTitle>
          <CardDescription>Select parameters to analyze historical data from exported CSV files.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
                 ...
                 <div className="flex flex-wrap gap-2">
                    {DEPARTMENTS.map(dep => (
                        <Button
                            ...
                            disabled={isFetching}
                        >
                            {isFetching && selectedDepartment === dep ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                            {dep}
                        </Button>
                    ))}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                    ...
                    <Select onValueChange={setSelectedSpcOption} value={selectedSpcOption} disabled={!selectedItem || spcOptions.length === 0}>
                        <SelectTrigger>
                            <SelectValue placeholder={
                                !selectedItem ? "Select an item first" :
                                spcOptions.length === 0 ? "No special characteristics found" :
                ...
                <div className="space-y-2">
                    <Label>5. Date Range of Exports</Label>
                    <Popover>
      ...
      {availableData !== null && !showCharts && !isLoading && (
        <Alert>
            ...
            <AlertDescription>
                Found {availableData} data points in the exported files for the selected criteria, but {numberOfPoints} are required for this analysis. Please select a larger date range or seed more data.
            </AlertDescription>
        </Alert>
      )}
      ...
      {showCharts && spcData && (
        <Card>
            <CardHeader>
                <CardTitle>Charts of {selectedItem?.itemName} - {spcOptions.find(o => o.id === selectedSpcOption)?.label}</CardTitle>
            </CardHeader>
            <CardContent>
