...
import { AtSign, Lock, UserCog, UserCheck, Loader2 } from "lucide-react";
import { 
  ...
  React.useEffect(() => {
    if (!loading && user) {
        ...
        getDoc(userDocRef).then(userDoc => {
            if (userDoc.exists()) {
                router.push(`/dashboard?role=${userDoc.data().role}`);
            } else {
                 toast({
                    title: "Error",
      ...
      <Card className="w-full max-w-md shadow-2xl">
        ...
        <CardFooter className="flex-col text-center text-xs">
          <p className="w-full text-muted-foreground">
            © {new Date().getFullYear()} Quality Check Inc. All rights reserved.
          </p>
          <p className="w-full text-muted-foreground">
            Made by Mateusz Kulig.
          </p>
        </CardFooter>
      </Card>
    </main>
