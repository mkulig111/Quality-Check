...
const createSubgroups = (data: number[], size: number) => {
  ...
  for (let i = 0; i < data.length; i += size) {
    ...
    if (subgroup.length === size) subgroups.push(subgroup);
  }
  return subgroups;
};

const D2_VALUES: { [key: number]: number } = {
  2: 1.128, 3: 1.693, 4: 2.059, 5: 2.326, 6: 2.534, 7: 2.704, 8: 2.847, 9: 2.970, 10: 3.078,
};
const D4_VALUES: { [key: number]: number } = {
...
export function SpcAnalysisCharts({ data, lsl, usl, subgroupSize = 5 }: SpcAnalysisChartsProps) {
  const {
    ...
    if (!data || data.length === 0) {
      return {
        overallMean: 0, overallStdDev: 0, withinStdDev: 0, cp: 0, cpk: 0,
        pp: 0, ppk: 0, ppm: 0, percentBelowLsl: 0, percentAboveUsl: 0,
    ...
    const A2 = 3.76 / (d2 * Math.sqrt(subgroupSize));
    ...
    const normalFn = (x: number) =>
      (1 / (overallStdDev * Math.sqrt(2 * Math.PI))) *
      Math.exp(-0.5 * Math.pow((x - overallMean) / overallStdDev, 2));
    const processDistributionData = bins.map((binData) => ({
      ...binData,
      curve: normalFn(binData.bin) * data.length * binWidth,
    }));

    return {
      overallMean, overallStdDev, withinStdDev, cp, cpk, pp, ppk, ppm,
      percentBelowLsl, percentAboveUsl, xBarChartData, rangeChartData, processDistributionData,
    };
          ...
          <CardContent>
            <ChartContainer config={chartConfig} className="h-[250px] w-full">
              <ComposedChart data={processDistributionData} barGap={0}>
                ...
                <XAxis dataKey="bin" type="number" domain={["dataMin", "dataMax"]} tickFormatter={tickFormatter} />
                <YAxis />
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              ...
              <TableRow><TableCell></TableCell><TableCell>Target</TableCell><TableCell className="text-right">{target.toFixed(3)}</TableCell></TableRow>
